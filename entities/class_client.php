...
';
		...
		public static function verifierEtudiant($id)
		{
			$req = config::$bdd->prepare("select type from  client where id=" . $id);
		...
		public static function envoyerMail( $id)
		{
			$req = config::$bdd->prepare("update  messagecontact set repondu='oui' where id=:id");
			$req = Bindvalue($req, array("id"), array($id));
			try {
				if ($req->execute()) {
		...
		public static function ajouterMessage($message, $mail, $nom, $prenom, $repondu)
		{
			$req = config::$bdd->prepare("insert into messagecontact(message,mail,nom,prenom,repondu) VALUE(:message,:mail,:nom,:prenom,:repondu)");
			$req = Bindvalue($req, array("message", "mail", "nom", "prenom", "repondu"), array($message, $mail, $nom, $prenom, $repondu));
			try {
				if ($req->execute()) {
